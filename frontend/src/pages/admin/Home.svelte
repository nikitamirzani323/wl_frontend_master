<script>
    ...
    const NewData = (e,username,pass,rule,name,status,create,update) => {
        sData = e
        if(sData == "New"){
            clearField()
        }else{
                                ...
                                {#each listAdmin as rec }
                                    <tr>
                                        <td NOWRAP style="text-align: center;vertical-align: top;cursor:pointer;">
                                            <i 
                                                on:click={() => {
                                                    NewData("Edit",rec.admin_username,"",rec.admin_rule,rec.admin_nama,rec.admin_status,rec.admin_create,rec.admin_update);
                                                }} 
                                        ...
                                        <td NOWRAP style="text-align: left;vertical-align: top;font-size: {table_body_font};">{rec.admin_create}</td>
                                        <td NOWRAP style="text-align: left;vertical-align: top;font-size: {table_body_font};">{rec.admin_update}</td>
                                    </tr>
                                {/each}
                            </tbody>
                            {:else}
        ...
        <div class="mb-3">
            <table>
                <tr>
                    ...
                    <td style="font-size: 11px;">{homecreate}</td>
                </tr>
                {#if homeupdate != ""}
                    <tr>
                        <td style="font-size: 11px;">Update</td>
                        <td style="font-size: 11px;">:</td>
                        <td style="font-size: 11px;">{homeupdate}</td>
                    </tr>
                {/if}
            </table>
        </div>
